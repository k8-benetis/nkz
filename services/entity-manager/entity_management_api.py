...
def upload_public_asset():
    """
    Upload a global asset (Platform Admin only).
    """
    try:
        # Check PlatformAdmin role
        user_roles = g.user_info.get('roles', [])
        # Also check realm_access for compatibility
        if not user_roles and 'realm_access' in g.user_info:
             user_roles = g.user_info['realm_access'].get('roles', [])
        ...
        # Validate file similar to tenant upload...
        filename = file.filename.lower()
        if asset_type == 'model':
            allowed_extensions = ['.glb', '.gltf']
            content_type = 'model/gltf-binary' if filename.endswith('.glb') else 'model/gltf+json'
        ...
        # Use original filename for public assets to make them recognizable, 
        # but sanitize it to avoid path issues
        safe_filename = "".join([c for c in file.filename if c.isalpha() or c.isdigit() or c in ['.', '-', '_']]).strip()
        s3_key = f"{PUBLIC_ASSETS_PREFIX}/{asset_type}/{safe_filename}"
